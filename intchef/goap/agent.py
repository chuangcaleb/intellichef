...
class BFSAgent(Agent):
    ...
    for neighbors of current:
      ...
      set neighbor's parent to current """
        ...
        self.opened_states = [recipe.ingredients]  # priority queue
        self.closed_states = {}  # empty set

        while self.opened_states:  # While not empty

            # sort opened list (No need to sort, all have a time step 1)

            # Pop opened into closed
            current_state = self.opened_states.pop(0)
            self.closed_states.add(current_state)
    ...
    def my_policy(self,
        ...
        # if self.solved == True:
        return self.optimal_actions_seq[timestamp]
        # else:
        #     pass
